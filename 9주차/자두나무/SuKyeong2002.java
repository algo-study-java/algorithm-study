    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 2; i <= time; i++) {
            for (int j = 0; j <= walk; j++) {
                ...
                /*
                 * 이동할 경우
                 * 현재 dp와 이전 초의 이전 이동횟수 dp 중 최댓값으로 dp 저장
                 * *** dp[i][j-1]가 아니고 dp[i-1][j-1]인 이유: 같은 초에서 이동횟수 변화는 불가능
                 * -> 예: dp[2][1] = Math.max(dp[2][1], dp[2][0]) -> 말도 안됨
                ...
                /*
                 * 이동하지 않을 경우
                 * (= i번째 나무의 번호와 현재 위치가 동일한 경우)
                 * 설정된 dp값에서 1씩 증가
                 * dp가 증가해야 하므로 마지막 순서에 if문 걸어야 함
                 */
